...
describe("Express App", () => {
  ...
  it("should process audio data for POST /receive", async () => {
    jest
      .spyOn(require("../Services/SpeechToText"), "speechToText")
      .mockResolvedValue("mock-transcription");
    jest
      .spyOn(require("../Services/PromptAI"), "callOpenAIWithTranscription")
    ...
    expect(response.text).toBe(
      "Audio received, transcribed, and processed successfully"
    );
    expect(
      require("../Services/SpeechToText").speechToText
    ).toHaveBeenCalledWith(audioData);
    expect(require("../Services/PromptAI").callOpenAI).toHaveBeenCalledWith(
      "mock-transcription"
    );
  });
  ...
  it("should return audio data for GET /send", async () => {
    ...
    expect(response.status).toBe(
      200
    ); /* generateAudioData(); not defined in app.js - Showing 500 status code instead*/
