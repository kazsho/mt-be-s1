...
describe('Controller functions', () => {
  ...
  describe('showConversation', () => {
    it('should return voice notes for a conversation', async () => {
      ...
      const mockedVoiceNotes = [{ id: 1, audio_data: 'mocked-audio-data' }];
      VoiceNotes.getByConversation.mockResolvedValue(mockedVoiceNotes); // Update to getByConversation
      ...
      expect(VoiceNotes.getByConversation).toHaveBeenCalledWith(conversationId); // Update to getByConversation
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith(mockedVoiceNotes);
    });
    ...
    it('should handle errors and respond with status 404', async () => {
      const conversationId = 1;
      VoiceNotes.getByConversation.mockRejectedValue(new Error('Conversation not found')); // Update to getByConversation
  ...
  describe('create', () => {
    it('should create a new voice note', async () => {
      ...
      const createdVoiceNote = { id: 1, audio_data: newVoiceNoteData.audio_data };
  ...
  describe('update', () => {
    it('should update a voice note', async () => {
      ...
      const updatedVoiceNote = { id: audioId, audio_data: updatedData.audio_data };
      VoiceNotes.getOneById.mockResolvedValue(voiceNote);
      voiceNote.update.mockResolvedValue(updatedVoiceNote); // Ensure that voiceNote.update is defined
